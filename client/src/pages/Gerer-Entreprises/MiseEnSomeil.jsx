...
const MiseEnSommeil = () => {
  ...
  useEffect(() => {
    const initProgress = async () => {
      try {
        ...
        const existing = res.data.find(p => p.typeDemarche === 'mise-en-sommeil' && !p.isCompleted);
  ...
  useEffect(() => {
    saveProgress();
  }, [currentStep, selectedEntreprise, formeSociale, timing, isArtisanale, observations]);

  const steps = useMemo(() => [
    {
      title: "Date de mise en sommeil",
      description: "Pour démarrer, quand souhaitez-vous mettre en sommeil votre entreprise ?",
      content: (
    ...
    {
      title: "Société à mettre en sommeil",
      description: "Quelle société souhaitez-vous mettre en sommeil ?",
      ...
      content: (
        <div className="space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Pour les activités artisanales, des formalités spécifiques peuvent s'appliquer pour la mise en sommeil.
          </p>
        </div>
      ),
    ...
    {
      title: "Observations",
      description: "Avez-vous des observations à faire sur votre dossier ? Une particularité dans votre dossier ?",
      content: (
        <textarea
          value={observations}
  ...
  ], [timing, selectedEntreprise, formeSociale, isArtisanale, observations]);
  ...
  try {
    ...
    navigate(`/paiement-fermeture/${nouveauDossier._id}`, {
      state: {
        ...
        type: 'MISE_EN_SOMMEIL',
        artisanale: isArtisanale // Pass this for payment calculation
      }
    });
      ...
      <div className="max-w-3xl mt-15 mx-auto px-4 py-8">
        ...
        <div className="bg-[#f4d47c]/10 rounded-xl p-5 border border-[#f4d47c]/30">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Notre équipe est disponible pour vous accompagner dans la mise en sommeil de votre société.
          </p>
           <button
