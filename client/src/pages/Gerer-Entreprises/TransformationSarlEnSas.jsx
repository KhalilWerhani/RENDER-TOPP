...
const TransformationSarlEnSas = () => {
  const { backendUrl } = useContext(AppContent);
  const navigate = useNavigate();

  const [currentStep, setCurrentStep] = useState(0);
  const [selectedEntreprise, setSelectedEntreprise] = useState(null);
  const [formeSociale, setFormeSociale] = useState('SARL');
  ...
  useEffect(() => {
    const initProgress = async () => {
      try {
        ...
        const existing = res.data.find(p => p.typeDemarche === 'transformation-sarl-sas' && !p.isCompleted);

        if (existing) {
          setCurrentStep(existing.currentStep);
          if (existing.formData) {
            setSelectedEntreprise(existing.formData.entreprise);
            setFormeSociale(existing.formData.formeSociale || 'SARL');
  ...
  const saveProgress = async () => {
    ...
    try {
      await axios.post(`${backendUrl}/api/progress/save`, {
        userId,
        typeDemarche: 'transformation-sarl-sas',
        currentStep,
        formData: {
  ...
  useEffect(() => {
    saveProgress();
  }, [currentStep, selectedEntreprise, formeSociale, modificationDate, beneficiaireEffectif, createdWithTop, modifiedStatuts, transformationObjectif, presenceCAC, isArtisanale]);

  const steps = useMemo(() => [
    {
      title: "Sélectionnez la société à transformer",
      description: "Recherchez votre SARL dans notre base de données",
      content: <SearchEntreprise onSelect={setSelectedEntreprise} />,
      validation: () => selectedEntreprise !== null
    },
    {
      title: "Transformation de SARL vers SAS",
      description: "Informations sur la transformation",
      content: (
        <div className="space-y-4">
          <p className="text-gray-600">
            Vous allez transformer votre société de forme SARL vers une SAS. Cette transformation implique une mise 
            à jour des statuts et la transmission d'un dossier complet au greffe.
          </p>
        </div>
      ),
      validation: () => true
        ...
        <div className="space-y-3">
          {[{ label: "Oui", value: true }, { label: "Non", value: false }].map(option => (
            <label key={option.label} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:border-[#f4d47c] cursor-pointer">
              <input
                type="radio"
                checked={presenceCAC === option.value}
                onChange={() => setPresenceCAC(option.value)}
                className="h-5 w-5 text-[#f4d47c] focus:ring-[#f4d47c]"
              />
              <span>{option.label}</span>
            </label>
    ...
    {
      title: "Objectif de la transformation",
      description: "Quel est l'objectif principal de cette transformation ?",
      content: (
        <select
        ...
        >
          ...
          <option value="Flexibilité juridique">Flexibilité juridique</option>
          <option value="Accès à des investisseurs">Accès à des investisseurs</option>
          <option value="Autre">Autre</option>
        </select>
      ),
      validation: () => transformationObjectif !== ''
    },
    {
    ...
    selectedEntreprise, modificationDate, beneficiaireEffectif, presenceCAC,
    transformationObjectif, createdWithTop, modifiedStatuts, isArtisanale
  ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: nouveauDossier } = await axios.post(
        `${backendUrl}/api/modification/transformation-sarl-sas/submit`,
        payload
      );
      ...
      navigate(`/paiement-modification?dossierId=${nouveauDossier._id}`, {
        state: {
          ...nouveauDossier,
          type: "TRANSFORMATION_SARL_EN_SAS"
        }
      });
      ...
      <div className="max-w-3xl mt-15 mx-auto px-4 py-8">
        ...
        <div className="bg-[#f4d47c]/10 rounded-xl p-5 border border-[#f4d47c]/30">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Notre équipe est disponible pour vous accompagner dans la transformation de votre SARL en SAS.
          </p>
...
export default TransformationSarlEnSas;