...
const FormAutoEntrepreneur = () => {
  const { backendUrl } = useContext(AppContent);
  const [currentStep, setCurrentStep] = useState(0);
  const [age, setAge] = useState('');
  const [nationaliteFr, setNationaliteFr] = useState(null);
  const [nationalite, setNationalite] = useState('');
  const [titreSejour, setTitreSejour] = useState(null);
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        const res = await axios.get(`${backendUrl}/api/progress/${id}/auto-entrepreneur`);
        const progress = res.data;

        if (progress) {
          const savedData = progress.formData;
          setAge(savedData.age || '');
          setNationaliteFr(savedData.nationaliteFr ?? null);
          setNationalite(savedData.nationalite || '');
          setTitreSejour(savedData.titreSejour ?? null);
          setCreationDate(savedData.creationDate || '');
  ...
  const saveProgress = async () => {
    ...
    try {
      await axios.post(`${backendUrl}/api/progress/save`, {
        userId,
        typeDemarche: 'auto-entrepreneur',
        currentStep,
        formData: {
          age,
          nationaliteFr,
  ...
  useEffect(() => {
    saveProgress();
  }, [currentStep, age, nationaliteFr, nationalite, titreSejour, creationDate, artisanale, adresse]);

  const steps = useMemo(() => {
    const baseSteps = [
      {
        title: "Quel est votre âge ?",
        description: "Pour vérifier votre éligibilité au statut d'auto-entrepreneur",
        content: (
          <div className="space-y-6">
            ...
            <p className="text-sm text-gray-500">
              <strong>Note :</strong> Un mineur de plus de 16 ans peut devenir auto-entrepreneur :
              <br />- S'il n'est pas émancipé : activités libérales non réglementées uniquement
              <br />- S'il est émancipé : toutes activités autorisées
            </p>
          </div>
        ),
        validation: () => age !== '' && parseInt(age) >= 16
      },
      {
        title: "Nationalité",
        description: "Êtes-vous de nationalité française ?",
        content: (
          <div className="grid grid-cols-1 gap-3">
            {[{label: "Oui", value: true}, {label: "Non", value: false}].map((option) => (
              <button
                key={option.label}
                onClick={() => setNationaliteFr(option.value)}
                className={`p-4 text-left rounded-lg border transition-all ${
                  nationaliteFr === option.value
        ...
        validation: () => nationaliteFr !== null
      },
      {
        title: "Date de création",
        description: "Quand souhaitez-vous créer votre auto-entreprise ?",
        content: (
          <div className="grid grid-cols-1 gap-3">
            {["Dans la semaine", "Dans les prochaines semaines", "Dans les prochains mois"].map((option) => (
              <button
                key={option}
                onClick={() => setCreationDate(option)}
                className={`p-4 text-left rounded-lg border transition-all ${
                  creationDate === option
                    ? 'border-[#f4d47c] bg-[#f4d47c]/10'
                    : 'border-gray-200 hover:border-[#f4d47c]'}`}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-3">
              {[{label: "Oui", value: true}, {label: "Non", value: false}].map((option) => (
                <button
                  key={option.label}
                  onClick={() => setArtisanale(option.value)}
                  className={`p-4 text-left rounded-lg border transition-all ${
                    artisanale === option.value
                      ? 'border-[#f4d47c] bg-[#f4d47c]/10'
                      : 'border-gray-200 hover:border-[#f4d47c]'}`}
                >
                  <span className="font-medium">{option.label}</span>
            ...
            <p className="text-sm text-gray-500">
              <strong>Exemples d'activités artisanales :</strong> boulangerie, maçonnerie, plomberie, coiffure, réparation, fabrication d'objets.
              <br /><strong>Exemples d'activités non artisanales :</strong> livreur de repas, chargeur de trottinettes.
            </p>
          </div>
        ),
      ...
      {
        title: "Adresse de votre auto-entreprise",
        description: "Où sera située votre activité principale ?",
        content: (
    ...
    // Insert nationality and residence permit steps if not French
    if (nationaliteFr === false) {
      baseSteps.splice(2, 0, 
        {
          title: "Votre nationalité",
          description: "Quelle est votre nationalité ?",
          content: (
            <select
              value={nationalite}
          ...
          validation: () => nationalite !== ''
        },
        {
          title: "Titre de séjour",
          description: "Possédez-vous un titre de séjour vous permettant de créer une entreprise ?",
          content: (
            <div className="grid grid-cols-1 gap-3">
              {[{label: "Oui", value: true}, {label: "Non", value: false}].map((option) => (
                <button
                  key={option.label}
                  onClick={() => setTitreSejour(option.value)}
                  className={`p-4 text-left rounded-lg border transition-all ${
                    titreSejour === option.value
                      ? 'border-[#f4d47c] bg-[#f4d47c]/10'
                      : 'border-gray-200 hover:border-[#f4d47c]'}`}
                >
                  <span className="font-medium">{option.label}</span>
          ...
          validation: () => titreSejour !== null
        }
      );
    }

    return baseSteps;
  }, [age, nationaliteFr, nationalite, titreSejour, creationDate, artisanale, adresse]);
    ...
    try {
      ...
      await axios.post(`${backendUrl}/api/dossier/questionnaire/${nouveauDossier._id}`, {
        reponses: {
          age,
          nationaliteFr,
          nationalite: nationaliteFr === false ? nationalite : 'Française',
          titreSejour: nationaliteFr === false ? titreSejour : null,
          creationDate,
          artisanale,
          adresse,
        },
      ...
      <div className="max-w-3xl mt-15 mx-auto px-4 py-8">
        ...
        <div className="bg-[#f4d47c]/10 rounded-xl p-5 border border-[#f4d47c]/30">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Nous sommes là pour vous aider à chaque étape de la création de votre auto-entreprise.
          </p>
...
export default FormAutoEntrepreneur;